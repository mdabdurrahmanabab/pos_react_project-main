...
function SupplierEdit() {
	...
	const getSupplier=(e)=>{
		axios.get(`${Constants.BASE_URL}/supplier/${params.id}`).then((res)=>{
			...
			getDistrict(res.data.data.district_id);
			getArea(res.data.data.area_id);
		}).catch((err)=>{
			console.log(err);
		});
	}
        	...
        	<div className="col-md-12">
        		<div className="card">
        			<div className="card-header">
        					<CardHeader title={'Edit Supplier'} link={'/supplier'} icon={'fa-list'} button_text={'List'}/>
        			</div>
			        						...
			        						<div className="row">
			        							<div className="col-md-6">
						        					<div className={"logo-preview mt-3"}>
						        						<img src={input.logo==undefined?input.logo_preview:input.logo} className="img-thumbnail aspect-one" alt={'Photo'}/>
						        					</div>
			        							</div>
			        						</div> : null
			        					}
	        					...
	        					<div className="row justify-content-center">
	        						<div className="col-md-4">
	        							<div className="d-grid mt-4">
	        								<button className="btn theme-button" onClick={handleSupplierUpdate} dangerouslySetInnerHTML={{__html: isLoading ? '<span class="spinner-grow spinner-grow-sm" role="status" aria-hidden="true"></span>Loading...' : "Update Supplier"}}/>
	        							</div>
	        						</div>
	        					</div>
	        				</div>
